defmodule Pisko.Elastix.Commits do
  ...
  def index(%{"sha" => sha} = commit) do
    Elastix.Document.index(get_env(:pisko, :elastic), @index_name, @doc_type, sha, commit)
  end

  @doc "Get commit from ElasticSearch index by commit sha"
  def get(sha) do
    body = Elastix.Document.get(get_env(:pisko, :elastic), @index_name, @doc_type, sha).body
